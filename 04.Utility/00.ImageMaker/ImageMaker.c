...
int main(int argc, char* argv[])
{
    ...
    if( argc < 4 )
    {
        fprintf( stderr, "[ERROR0] ImageMaker.exe BootLoader.bin Kernel32.bin Kernel64.bin\n" );
        exit( -1 );
    }
    ...
    if( ( iTargetFd = open( "Disk.img", O_RDWR | O_CREAT |  O_TRUNC |
            S_IREAD | S_IWRITE ) ) == -1 )
    {
        fprintf( stderr , "[ERROR1] Disk.img open fail.\n" );
        exit( -1 );
    }
    ...
    {
        fprintf( stderr, "[ERROR2] %s open fail\n", argv[ 1 ] );
        exit( -1 );
    }
    ...
    {
        fprintf( stderr, "[ERROR3] %s open fail\n", argv[ 2 ] );
        exit( -1 );
    }
    ...
    {
        fprintf( stderr, "[ERROR4] %s open fail\n", argv[ 3 ] );
        exit( -1 );
    }
...
int CopyFile( int iSourceFd, int iTargetFd )
{
    ...
    while( 1 )
    {
        ...
        if( iRead != iWrite )
        {
            fprintf( stderr, "[ERROR5] iRead != iWrite.. \n" );
            exit(-1);
        }
